...
**Batch 9 — Dashboard/History trust surface (P0, live UI walkthrough 2026-09-11) — scoped, plan at `docs/superpowers/plans/2026-09-11-dashboard-history-trust-surface.md`**
- Aggregate + per-event-type accuracy rollup on the History tab (the same breakdown already hand-derived via SQL twice — `docs/fundamental-analysis-review-2026-09-11.md` §3 — should be a standing feature)
- Low-signal-coverage warning badge on the drill-down panel's existing "no individual article stood out" state (the general UI-side counterpart to Batch 2's CPI-specific keyword fix — scoped down from a general "relevance/quality" indicator during implementation planning, since detecting relevance for events with no `EVENT_RELEVANCE_KEYWORDS_BY_TITLE` entry would require new keyword curation per event, a content task out of scope here, not a UI one)
- Surface the Tier1-vs-sentiment conflict field (Batch 4) in the History tab, not just the Dashboard card
- Fix the `events[0]`-only Dashboard rendering limit (found during Batch 4's live verification, previously logged with no batch home — assigned here) so a real conflict or signal doesn't go invisible the moment its event scrolls out of the featured cluster
...
Touches the same files Batch 3/4/5 already cleared debt in (`webapp/predictions_service.py`, `webapp/app.js`, `webapp/history.py`, `webapp/app.py`) — no dependency on Batch 6/7/8, but **recommended to ship before Batch 6** despite the higher number, same "clear room before the next signal source" reasoning as Batches 3/5: Tier 2/3 will add yet more card content that makes the events[0]-only visibility limit and the missing-rollup problem worse, not better, if left unfixed first. Batch numbers are append-only per this file's own convention (existing numbers map to already-shipped commits) — sequencing note lives here in prose instead of renumbering.
