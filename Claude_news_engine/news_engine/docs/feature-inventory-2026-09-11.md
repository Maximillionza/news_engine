...
| Only `events[0]` and its exact-timestamp siblings ever render on the Dashboard tab — every other event for a tracked symbol (a different day/cluster) is completely invisible there, regardless of signal type | **Shipped** (Batch 9) | `webapp/static/app.js`'s `otherTrackedEventsHtml()` — the real Sep 10 PPI Tier1-vs-sentiment conflict now surfaces here. Doc correction 2026-09-11: this row was left marked Outstanding after Batch 9 actually shipped it |
