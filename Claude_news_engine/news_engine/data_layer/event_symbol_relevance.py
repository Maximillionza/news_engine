...
_RELEVANCE_TABLE: dict[tuple[str, str], RelevanceJudgment] = {
    ...
    ("ISM Manufacturing PMI", "US30"): RelevanceJudgment(
        RelevanceStatus.RELEVANT,
        "CORRECTED 2026-09-16, round 2 -- two successive news-article "
        "citations for this entry were independently knocked down by "
        "review (first a misdated MarketPulse article, then a Dow "
        "rally figure that turned out to be Aug 4 2026, not Aug 3 2026, "
        "per five independently corroborating sources naming Strait-of-"
        "Hormuz deal hopes and Caterpillar/Palantir earnings, not ISM, "
        "as the driver). Per explicit review guidance, this entry now "
        "rests on internal DB evidence ALONE, with no external news-"
        "article 'driver' narrative layered on top: 2 real ISM "
        "Manufacturing PMI occurrences logged against US30 in this "
        "project's own scoring DB (scoring/backtest_log.db, outcomes "
        "table, event_title='ISM Manufacturing PMI', instrument='US30') "
        "-- row id 55 (2026-03-02T15:00:00Z, actual_direction 'bullish', "
        "actual_move_note 'Dukascopy: +0.23% in 30min (auto)', source "
        "'seeded') and row id 87 (2026-09-01T14:00:00Z, actual_direction "
        "'bullish', actual_move_note 'Dukascopy: +0.41% in 30min "
        "(auto)', source 'live') -- both are real, ISM-specific, "
        "Dukascopy-measured price moves in the 30-minute window "
        "immediately following the release itself, not a same-day close "
        "confounded by later unrelated news",
    ),
    ("ISM Manufacturing PMI", "US500"): RelevanceJudgment(
        RelevanceStatus.UNVERIFIED,
        "CORRECTED 2026-09-16, round 2. Checked this project's own "
        "scoring DB directly (scoring/backtest_log.db) for real internal "
        "evidence the same way US30's entry uses it: "
        "SELECT * FROM outcomes WHERE event_title LIKE '%ISM%' AND "
        "instrument='US500' returns zero rows; SELECT * FROM predictions "
        "WHERE event_title LIKE '%ISM%' AND instrument='US500' also "
        "returns zero rows -- no internal DB evidence exists for this "
        "pair. Two separate WebSearch/WebFetch passes for external "
        "evidence each produced a citation that a subsequent review "
        "independently disproved as misdated/misattributed (see US30's "
        "citation history above for the same underlying pattern) -- both "
        "attempts named a specific S&P 500 rally day and credited it to "
        "ISM, and both were shown, by independently corroborating "
        "sources, to be either the wrong date or a rally the sources "
        "themselves attribute to other named drivers (oil, Strait-of-"
        "Hormuz deal hopes, earnings) with no mention of ISM. Per "
        "explicit review guidance, not attempting a third external "
        "citation -- honestly marking this UNVERIFIED: no internal DB "
        "evidence and no independently-corroborated external case of an "
        "ISM print's own reaction in US500 found after two research "
        "passes",
    ),
    ("ISM Manufacturing PMI", "NAS100"): RelevanceJudgment(
        RelevanceStatus.UNVERIFIED,
        "CORRECTED 2026-09-16, round 2 -- the prior version of this "
        "entry's citation referenced an 'Aug 3 2026' Nasdaq Composite "
        "+2.59% figure as background; that date has since been shown by "
        "independent review (five corroborating sources: CBS News, Al "
        "Jazeera, Bloomberg, Qz, Benzinga) to actually be Aug 4 2026, "
        "attributed by those sources to Strait-of-Hormuz deal hopes and "
        "Caterpillar/Palantir earnings, not ISM -- so that reference is "
        "dropped rather than corrected-and-kept, per review guidance not "
        "to re-attempt a news-article 'driver' narrative for this event "
        "type's equity-index entries. No internal DB evidence exists for "
        "NAS100 either (scoring/backtest_log.db has no ISM Manufacturing "
        "PMI rows for this instrument, checked directly). Remains "
        "UNVERIFIED: no internal DB evidence and no independently-"
        "corroborated external case of an ISM print's own reaction in "
        "NAS100 (as distinct from the Nasdaq Composite) found after two "
        "research passes",
    ),
    ...
    ("Retail Sales m/m", "NAS100"): RelevanceJudgment(
        ...
        "Retail Sales m/m. CORRECTED 2026-09-16 after review: an earlier "
        "version of this citation claimed no dated, NAS100-specific, "
        "post-release source naming Retail Sales as the driver was found -- "
        "that was wrong; a real one exists and was missed on the first pass. "
        "Re-searched and independently re-fetched directly: "
        "macrostructuredaily.substack.com, 'Nasdaq & S&P 500 Futures: "
        "Post-Retail Sales Update into the New York Cash Open' (2026-08-14) "
        "states 'Nasdaq futures briefly weakened ahead of the release, then "
        "responded sharply higher once the data hit' and that NQ 'reclaimed "
        "the upper part of the intraday map' post-release -- a real, dated, "
        "NAS100/NQ-specific, post-release claim, not reasoned by analogy "
        "from the Nasdaq Composite. Judged insufficient to upgrade to "
        "RELEVANT on its own, though: it is a single retail-macro Substack "
        "post, not a major financial outlet (the standard every other "
        "RELEVANT entry in this table meets), gives no specific price level "
        "or percentage move to check, and its 'sharply higher' language "
        "conflicts in degree (though not direction) with the only "
        "quantified NAS100 figure found elsewhere -- WebSearch confirms "
        "NAS100 futures at 30,233.00, up a modest 0.15% immediately after "
        "the 8:30am ET release, not a 'sharp' move. Also still checked and "
        "unresolved: the Nasdaq Composite's own eventual cash-session close "
        "that day was DOWN 0.28-0.5%, the opposite direction from the "
        "futures' intraday recovery -- a genuinely mixed signal across the "
        "session, not a single clean directional NAS100 move. Honest "
        "UNVERIFIED: one real but thin, unquantified, magnitude-inconsistent "
        "source is not enough to force a RELEVANT call, but the search that "
        "found it (and what it says) is now accurately described here",
    ),
