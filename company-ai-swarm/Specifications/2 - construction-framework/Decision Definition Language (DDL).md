...
domain, no existing equivalent elsewhere in Specifications/. Decision lifecycle (§7) is
adopted as the canonical lifecycle for the Decision object type (see Universal Ontology §5).
...
Every Decision SHALL inherit from EnterpriseObject (Universal Ontology §2-3).
...
Authority: Legal and Compliance.
...
Note: this is the decision-escalation chain, distinct from AOSS's Authority Level scale
(an agent's assigned organizational rank). A single agent's decisions may need to escalate
through several of these tiers depending on the decision's impact rating (§12), regardless
of that agent's own Authority Level.
...
Generalizable patterns may enter Memory only after validation (see EMAS §15 Memory Promotion).
