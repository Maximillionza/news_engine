...
Imported from the repository-documents reconciliation pass, as the dedicated architecture
document for the COO role that AOSS, DOMS, TCAIS, EIB, and RCS all reference but none fully
define. A companion document, "COO Orchestrator Algorithm Specification (COO-AS)," covers
similar ground with a slightly different (and equally unweighted) complexity formula; that
document was judged redundant and was not imported — this is the canonical COO specification.
...
Input: "Create a market entry strategy."
...
Required Capabilities: Research, Finance, Marketing, Risk Analysis
```
...
```
Summarise document → Complexity: 1
Create legal strategy → Complexity: 4
```

This is a factor checklist, not a scoring formula — no numeric weights, per-factor scales,
or score-to-level thresholds are defined anywhere in the corpus. The worked example above
asserts final scores without showing the arithmetic. Treat this as the input list for a
complexity-classification decision, not an executable algorithm, until Configuration/models/
defines concrete weights and thresholds.
...
Objective: "Build customer data platform."
...
```
Database Design → Engineering Department
...
Available: Security Agent A, Security Agent B
Selection: Agent B
Reason: Higher architecture capability score
```
...
```
Task: Extract names from document → Model: Efficient model
Task: Design enterprise architecture → Model: Advanced reasoning model
```
...
```
Objective: Create cybersecurity strategy

Workflow:
1. Research threat landscape
2. Analyse current environment
3. Design controls
4. Compliance review
5. Quality review
6. Deliver strategy
...
The COO SHALL determine tasks that can execute simultaneously.
...
Cannot perform Security Audit before System Architecture exists.
...
The COO SHALL NOT blindly learn "Previous project failed, therefore...".

Memory stored SHALL instead include:

```
Context: Previous project type
Environment: Previous constraints
Lesson: Specific improvement
Applicability: Defined conditions
```
...
This mirrors the Decision Ledger in Decision Definition Language §17 — a COO Decision
Record is a Decision Ledger entry scoped to operational (COOS §6.3-class) decisions.
...
Previous: Healthcare Data Project, High POPIA requirements.

New: Public Marketing Website, Different risk profile.
...
Level 2  Execution with approval
Level 3  Controlled autonomy
Level 4  Enterprise autonomy
```

Note: this is a COO-specific autonomy scale, distinct from AOSS §28's general Agent Autonomy
Levels (1-5). The COO, as a singular enterprise role, uses this 4-level scale; individual
agents use AOSS's 5-level scale.
